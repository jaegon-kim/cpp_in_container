...
namespace a02_07 {
    ...
    void const_graph(const int input[][MAX_M], int capa[][MAX_NODE], int size_n, int s, int t) {
    ...
    void test(const int input[][MAX_M], int size_n, int size_m) {
        memset(capa_g, 0, sizeof(capa_g));
        memset(flow_g, 0, sizeof(capa_g));
        memset(final_g, 0, sizeof(capa_g));

        const_graph(input, capa_g, size_n, s_node(), t_node(size_n, size_m));
