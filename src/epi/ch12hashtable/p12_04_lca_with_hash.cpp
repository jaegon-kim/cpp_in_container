...
namespace p12_4 {
    ...
    node_ptr find_lca_with_hash(node_ptr root, node_ptr n0, node_ptr n1) {
        unordered_set<int> set0;
        unordered_set<int> set1;

        while(true) {
            if (set1.find(n0->data) != set1.end()) {
                return n0;
            }
            if (set0.find(n1->data) != set0.end()) {
                return n1;
            }
            ...
            if (n0 != root) {
                set0.emplace(n0->data);
                n0 = n0->parent;
            }

            if (n1 != root) {
                set1.emplace(n1->data);
                n1 = n1->parent;
            }
        }
    ...
    void test() {
        ...
        lca_h = find_lca_with_hash(root, node_0, node_1);
        cout << (lca? lca->data: -1) <<" " << (lca? lca->data: -1) << endl;

        lca = find_lca(root, node_1, node_2);
        lca_h = find_lca_with_hash(root, node_1, node_2);
        cout << (lca? lca->data: -1) <<" " << (lca? lca->data: -1) << endl;

        lca = find_lca(root, node_4, node_0);
        lca_h = find_lca_with_hash(root, node_4, node_0);
        cout << (lca? lca->data: -1) <<" " << (lca? lca->data: -1) << endl;
...
void test_p12_04_lca_with_hash() {
    PRINT_FUNC_NAME;
    p12_4::test();
}
